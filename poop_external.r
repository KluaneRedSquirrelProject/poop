...
pfile_2009 <- read_xlsx("data/krsp_fecal-samples_2008-12.xlsx") %>% 
  filter(Year %in% 2009:2011, !is.na(Poop)) %>% 
...
matches <- multiples %>% 
  ...
  select(-loc_match) %>% 
  bind_rows(matches, .)
multiples <- filter(multiples, !id %in% matches$id)
...
# combine problems
problems <- bind_rows(problem_2006, problem_2007, problem_2009)
...
# combine poop
poop <- bind_rows(poop_2006, poop_2007, poop_2009) %>% 
